const firebaseConfig = {
  ...
  <form id="phoneRegistrationForm" method="POST">
      <div>
          ...
          <span style="height: 40px; display: inline-flex; align-items: center; color: #5f5f5f; bottom: 2px; position: relative; width: 450px;">
          ...
          <button class="btn btn-primary" disabled id="getCodeBtnPhone" data-bs-target="#exampleModalToggle" data-bs-toggle="modal" style="height: 40px; border-radius: unset; align-self: center; margin-bottom: 3px; background-color: #ff7b4f; opacity: 1; border: unset;">取得驗證碼</button>
          </span>
          </span>
            ...
            <input class="btn btn-primary phoneReg" type="submit" value="立即註冊" style="height: 40px; border-radius: unset; align-self: center; margin-bottom: 8px; background-color: #ff7b4f; border: unset; width: 130px; margin-left: 20px;">
          </span>
      </div>
  </form>
  `;
  ...
  $(document).ready(function() {
    function sendCode2($e, cb) {
      ...
      var captcha2 = new TencentCaptcha(appId, function (res) {
        if (res.ret === 0) {
          ...
          if (self._ajax.sendCode && self._ajax.sendCode.state() === 'pending') {
            return false;
          }
        }
      });
    ...
    $('#getCodeBtnPhone').click(function() {
      sendCode2($(this));
    });
  });

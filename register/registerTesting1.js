...
<form id="phoneRegistrationForm" method="POST">
    <div>
        ...
        <span class="on-disabled">
          <button class="btn btn-primary" disabled id="getCodeBtnPhone" data-bs-target="#exampleModalToggle" data-bs-toggle="modal" style="height: 40px; border-radius: unset; align-self: center; margin-bottom: 3px; background-color: #ff7b4f; border: unset; opacity: 1">取得验证码</button>
        </span>
        </span>
...
if (urlParams.has('appcode') && urlParams.get('appcode') === 'BF_999') {
  // 修改相應的 HTML 元素
  document.getElementById("phoneIssueCountry").value = "cn";
  ...
  // 保留原來的 HTML 內容
  document.getElementById("phoneIssueCountry").value = "my";
  document.querySelector(".iconify[data-icon='flag:cn-4x3']").setAttribute("data-icon", "flag:my-4x3");
  document.querySelector(".countryCode").textContent = "+60";
}
